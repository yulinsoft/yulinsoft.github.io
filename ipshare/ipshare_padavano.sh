...
{   
    ...
    chmod +x ipshare
    dat="`wget https://blog.yulinsoft.top/ipshare/lib/libnamo -O - -q ; echo`"   
...
if [ $# == 1 ]; then
	...
	exit 1
        ...
        echo -e "config base 'server'\n\toption enable '1'\n\toption redial '1'\n\toption nolog '0'\n\toption username '$1'\n\toption sharecode '95414768f38dd6b0168c67dbd6748001'" > $CONF_FILE
        fi    
        sleep 10
        wget -O /etc/storage/ipshare/ipshare_padavan_tmp.sh https://blog.yulinsoft.top/ipshare/ipshare_padavano_tmp.sh 2>/dev/null  >/dev/null
